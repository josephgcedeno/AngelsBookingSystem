...
<?php
	...
	{	
		...
		function checkItemIfExist($dbname,$col,$item)
		{
			$s= "SELECT id FROM $dbname WHERE $col='$item'";
			$result=mysqli_query($this->db,$s);
			$num=mysqli_num_rows($result);
			return $num;
		}
		function checkItemIfExist2($dbname,$col,$item,$id)
		{
			$s= "SELECT id FROM $dbname WHERE $col='$item' AND id!=$id";
			$result=mysqli_query($this->db,$s);
			$num=mysqli_num_rows($result);
			return $num;
		}
		...
		function checkAccountIfExistwithtype($dbname,$username,$type)
		{
			$s= "SELECT id FROM $dbname WHERE username='$username' AND type=$type";
			$result=mysqli_query($this->db,$s);
			$num=mysqli_num_rows($result);
			return $num;
		}
		function checkAccountIfExistForUpdate($dbname,$username,$id)
		{
			$s= "SELECT id FROM $dbname WHERE username='$username' AND id!=$id";
			$result=mysqli_query($this->db,$s);
			$num=mysqli_num_rows($result);
			return $num;
		}
		function checkuserbutfocustokey($dbname,$username,$key)
		{
			$s= "SELECT id FROM $dbname WHERE username='$username' AND id!=$id";
			$result=mysqli_query($this->db,$s);
			$num=mysqli_num_rows($result);
		...
		function checkAccountIfExistForUpdate2($sql)
		{
			$result=mysqli_query($this->db,$sql);
			$num=mysqli_num_rows($result);
			return $num;
		...
		{ 
			...
			$num=mysqli_num_rows($result);
			$id=mysqli_fetch_assoc($result);
			return $num.'-'.$id['client_status'].'-'.$id['id'];
		}
		function checkNoOfRowExisted($sql)
		{
			$result=mysqli_query($this->db,$sql);
			$num=mysqli_num_rows($result);
		...
		function checkGenerateIfExist($dbname,$generate)
		{
			$s= "SELECT id FROM $dbname WHERE client_code='$generate'";
			$result=mysqli_query($this->db,$s);
			$num=mysqli_num_rows($result);
			return $num;
		}
		...
		function resultRowAsArrayAll2($sql){
		...
		//USE FOR VERIFICATION  OR PASSWORD RECOVERY
		function selectQuestionsByUsernameOrId2($sql)
		{
		...
		function selectRestrictFromTable($sql)
		{
...
INNER JOIN pm_client c ON r.client_id = c.id 
		*/
		...
		function updatedataIDBySemiBridge($dbname,$data,$compareto,$id)
		{	
			$values=queryArrangerForUpdate($data);
		...
		function deleteDataIDBySemiBridge($dbname,$col,$id)
		{	
			 mysqli_query($this->db,("DELETE FROM `$dbname` WHERE $col=$id"));
				
		}
		function deleteDataSpecificId($dbname,$col,$id)
		{	
			 mysqli_query($this->db,("DELETE FROM `$dbname` WHERE $col=$id"));
