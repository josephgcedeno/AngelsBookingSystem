...
/*
AUTO LOADER WITH SCROLLING
$(document).ready(function(){
              if (action=='inactive') 
              {
                  action='active';
                  loadproducts(limit,start);
              }
              $(window).scroll(()=>
              {
                  
                    if ($(window).scrollTop()+$(window).height()/ $(`#grid`).height() > $(`#grid`).height() 
                        && action=='inactive') 
                    {  
                       active='active';
                       start= start+limit;
...
$(document).ready(()=>
{
 ...
 function loadproducts(limit,start)
  {
            
      $.ajax(
      {
          url: "process/fetchproduct.php",
          type: "POST",
          data: 
          {
             limit:limit,
          ...
          success:(result)=>
          {
            ...
            let display=result.split(`separatethis`);

              if (display[1]=='' || $(`#grid`).html()=='') 
              {
                action="active";
                counter=1;
              }
              ...
              {      
                 ...
                 /*   $(`#grid`).find('script[src="js/menu.js"]').remove();
                    $('#grid').find('script').filter(function(){
                                      return $(this).attr('src') === 'js/menu.js'
                                       }).remove();
                    $('#grid').find('script[src="js/menu.js"]').remove();
                    $('#grid').find('script').attr('src', 'js/menu.js').remove();
